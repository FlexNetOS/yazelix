  ...
  in {
    ...
    packages = eachSystem (system: let
      ...
      flexnetosCargoPgrx = pkgs.rustPlatform.buildRustPackage {
        ...
        installPhase = ''
          runHook preInstall
          cargo_pgrx_bin=$(find target -type f -perm -0100 -name 'cargo-pgrx*' -print -quit)
          test -n "$cargo_pgrx_bin"
          install -Dm755 "$cargo_pgrx_bin" "$out/bin/cargo-pgrx"
          runHook postInstall
        '';
      };
      ...
      in pkgs.runCommand "flexnetos-foundation-contracts" {} ''
        ...
        ${pkgs.jq}/bin/jq -e '
          [.hooks.PreToolUse[]
            | select(
                .matcher == "*"
                and any(.hooks[]; .command == "/home/flexnetos/.nix-profile/bin/agent guard")
              )]
          | length == 1
        ' "$codex_test_runtime/hooks.json"
        ...
        ${pkgs.jq}/bin/jq -e '
          [.hooks.PreToolUse[]
            | select(
                (has("matcher") | not)
                and any(.hooks[]; .command == "/home/flexnetos/.nix-profile/toolbin/agent guard")
              )]
          | length == 1
        ' "$claude_test_runtime/settings.json"
        ...
        grep -F '`rtk bun x --bun gitnexus@latest analyze`' "$gitnexus_test_runtime/AGENTS.md"
