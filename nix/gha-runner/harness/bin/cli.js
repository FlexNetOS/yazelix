...
// Generated by metaharness — the `flexnetos-runner` CLI entry point.
//
// This is plain ESM JavaScript on purpose: it runs as-is via `npx flexnetos-runner`
...
// "bin" + "files" fields in package.json), so `npx flexnetos-runner` works the moment
// `npm install` has resolved @metaharness/kernel + @metaharness/host-github-actions.

import { loadKernel } from '@metaharness/kernel';
import adapter from '@metaharness/host-github-actions';

const HARNESS_NAME = 'flexnetos-runner';

/** `flexnetos-runner init` — boot the kernel + host adapter and report status. */
async function init() {
...
/** `flexnetos-runner doctor` — verify the install end-to-end (kernel + host resolve). */
