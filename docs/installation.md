...
```sh
nix profile add --refresh --profile <tmp>/yazelix-variant-profile github:FlexNetOS/yazelix#yazelix-no-helix
```
...
```sh
nix profile add --refresh --profile <tmp>/yazelix-variant-profile github:FlexNetOS/yazelix#yazelix-no-mars
```
...
```sh
nix profile add --refresh --profile <tmp>/yazelix-variant-profile github:FlexNetOS/yazelix#yazelix-no-yazi
```

The modifiers compose mechanically:

```sh
nix profile add --refresh --profile <tmp>/yazelix-variant-profile github:FlexNetOS/yazelix#yazelix-no-mars-no-helix-no-yazi
```
...
The default `#yazelix` and the `yazelix-no-*` variant outputs are supported
evaluation alternatives, not additional owners of the FlexNetOS foundation
profile.
...
For a standard (non-foundation) profile install, pass the installed package
name to `nix profile upgrade --refresh`. Confirm a foundation entry name with:
