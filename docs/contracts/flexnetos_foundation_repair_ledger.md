...
| RTK | FlexNetOS RTK 0.43.0 fork | FlexNetOS release package in foundation | `github:FlexNetOS/rtk-tokenkill/5e0f065362ece172899a1bf3da82ac0eaada6a1a` via `packaging/rtk_release.nix` | foundation extra runtime package | `rtk` | `rtk --version` | native Nu module | Repaired | Keep the fork revision explicit in `flake.lock`; do not restore an upstream or user-bin shadow | profile-owned `rtk --version` reports `rtk 0.43.0`; installed binary hash matches the fork-pinned Nix build |
| rtk-tokenkill fork | Active foundation source | Release-metadata-synced FlexNetOS fork | `src/rtk-tokenkill` | `rtk_source` | `rtk` | `git rev-list upstream_tmp/v0.43.0...HEAD` | native Nu module | Active | Preserve fork-only code and dependency upgrades while importing upstream releases without downgrade | PR #7 syncs the upstream v0.43.0 release metadata onto the FlexNetOS fork |
...
| bun | LifeOS/Yazelix foundation toolchain | Profile export | `pkgs.bun` via `packaging/flake_outputs.nix` | `lifeos_foundation_yzx` | `bun` | `bun --version` | n/a | Profile-owned | Keep the profile export; workspace-root `/home/flexnetos/FlexNetOS/usr/bin/bun` is a migration target, not ownership proof | package build proof resolves `$out/bin/bun` to Nix store `bun-1.3.13` |
| bunx | LifeOS/Yazelix foundation toolchain | Profile export | `pkgs.bun` via `packaging/flake_outputs.nix` | `lifeos_foundation_yzx` | `bunx` | `bunx --version` | n/a | Profile-owned | Same as bun | package build proof resolves `$out/bin/bunx` to Nix store `bun-1.3.13` |
| kache | FlexNetOS/Yazelix Rust build accelerator | Profile export | `packaging/kache_release.nix` plus wrapper in `packaging/flake_outputs.nix` | `lifeos_foundation_yzx` | `kache` | `kache --version` | n/a | Profile-owned | Use the profile package for Yazelix/FlexNetOS Rust builds; workspace-root `/home/flexnetos/FlexNetOS/usr/bin/kache` is a migration target | package build proof resolves `$out/bin/kache` to `kache 0.8.0` |
| wild linker | FlexNetOS/Yazelix Rust linker | Profile export | `pkgs.wild` via `packaging/flake_outputs.nix` | `lifeos_foundation_yzx` | `wild`, `ld.wild` | `wild --version`; `ld.wild --version` | n/a | Profile-owned | Use the profile-owned linker with the profile-owned Rust toolchain; workspace-root wild shims are migration targets | package build proof resolves `$out/bin/wild` and `$out/bin/ld.wild` to Wild 0.9.0 |
...
| tauri | LifeOS native build helper | Profile export plus project scripts | `pkgs.cargo-tauri` via `packaging/flake_outputs.nix`; project scripts stay in `src/lifeos/package.json` | `lifeos_foundation_yzx` for host CLI | `cargo-tauri` | `cargo-tauri --version` | n/a | Profile-owned host CLI | Keep project-local JS scripts for app builds, but do not rely on ambient PATH for the native CLI | package build proof resolves `$out/bin/cargo-tauri` to tauri-cli 2.11.4 |
...
2. Keep the active foundation on the release-synced FlexNetOS `rtk-tokenkill`
   fork. Pin its exact revision in the Yazelix flake and expose it only through
   the single profile-owned binary plus the native Nushell routing module.
3. Export bun and bunx from lifeos_foundation_yzx. Treat any workspace-root
   /home/flexnetos/FlexNetOS/usr/bin bun/bunx wrappers as migration targets,
   not ownership proof.
4. Export kache and wild/ld.wild from lifeos_foundation_yzx for Yazelix/FlexNetOS
   Rust builds. Keep meta as a workspace control-plane command until it has a
   profile package contract.
5. Keep vue and vite project-local to LifeOS and invoked through Bun scripts.
   Export the native Tauri host CLI as cargo-tauri from the profile.
