...
| YZX-BP-004 | Editable input, generated proof, and installed frontdoor have one owner | `yazelix`: paths/runtime contracts, profile tooling | editable `~/.config/yazelix`; generated `~/.local/share/yazelix`; executable `/home/flexnetos/.nix-profile/bin/yzx` | single-profile contract, `yzx inspect --json`, command/type/symlink/profile receipts | Implemented |
...
| YZX-BP-008 | Yazi remains the navigation surface; every documented plugin/config sidecar path is available (§4.3, row 6089) | `yazelix-yazi-assets`; main Yazi materializer and contracts | one child package owns auto-layout, Git, Starship plugins/config, sidebar plugins, optional native TOML/Lua/keymap/theme/package layers | child package tests; `yzx_yazi_materialization`; main config/open contracts | Implemented |
...
| YZX-BP-022 | PR #77 has a definitive provenance-preserving disposition | `FlexNetOS/yazelix` GitHub history | original pull ref retained; exact semantic and attribution already merged through PR #81 | PR metadata/timeline/diff, `cd7379ff…`, current README, final explanatory close | Completion gate |
...
| Yazi native files/plugins | TOML, Lua, keymap, package, theme, Starship, auto-layout, Git, sidebar state/status, zoxide editor | child package plus materialization check |
