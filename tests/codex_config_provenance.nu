# Runtime provenance gate for Codex config, durable rules, and hooks authorship.
#
# Usage:
...
def main [
    ...
    --deployed-hooks-src: path = "/home/flexnetos/.nix-profile/share/yazelix/agent_configs/codex/hooks.json.src"
] {
    ...
    let repo_hooks_src = ($root | path join "agent_configs/codex/hooks.json.src")
    let live_config = ($config_home | path join "config.toml")
    let live_rules = ($config_home | path join "RULES.md")
    let live_hooks = ($config_home | path join "hooks.json")
    mut failures = []
    ...
    let required_inputs = [
        ...
        {name: "deployed-hooks", path: $deployed_hooks_src}
        {name: "review-config", path: $repo_config_src}
        {name: "review-rules", path: $repo_rules_src}
        {name: "review-hooks", path: $repo_hooks_src}
    ]
    ...
    for pair in [
        ...
        {name: "hooks", deployed: $deployed_hooks_src, review: $repo_hooks_src}
    ] {
    ...
    if ($prerequisites | all {|value| $value | path exists }) {
        ...
        let expected_hooks = ($workdir | path join "hooks.json")
        ...
        cp $live_hooks $expected_hooks
        let result = (do { ^$nu.current-exe $materializer $deployed_config_src $expected_config $deployed_rules_src $expected_rules $deployed_hooks_src $expected_hooks } | complete)
        if $result.exit_code != 0 {
            ...
            for pair in [
                ...
                {name: "hooks", expected: $expected_hooks, live: $live_hooks}
            ] {
    ...
    if ($live_hooks | path exists) {
        let raw = (open --raw $live_hooks)
        let parsed = (try { $raw | from json } catch { {} })
        let description = ($parsed.description? | default "")
        let marked = ($description | str contains "GENERATED by yazelix codex hooks materializer")
        let source_hash = (if ($deployed_hooks_src | path exists) { open --raw $deployed_hooks_src | hash sha256 } else { "" })
        let hashed = (not ($source_hash | is-empty)) and ($description | str contains $"source_sha256 = ($source_hash)")
        let mode = (ls -l $live_hooks | get mode.0)
        ...
        let commands_exact = ($required_commands | all {|command|
        ...
        let posttool_exact = (try {
            $hook_map.PostToolUse | any {|group|
                let matcher_exact = ($group.matcher? | default "") == "^Bash$"
                let command_exact = ($group.hooks | any {|handler| ($handler.command? | default "") == "/home/flexnetos/.nix-profile/bin/icm hook post" })
                $matcher_exact and $command_exact
            }
        } catch { false })
        if $marked and $hashed and ($mode == "rw-r--r--") and $commands_exact and $pretool_exact and $posttool_exact {
            print "clause hooks-generated-profile-owned: ok"
        } else {
            $failures = ($failures | append "hooks-generated-profile-owned")
            print $"clause hooks-generated-profile-owned: FAIL marker=($marked) source-hash=($hashed) mode=($mode) commands=($commands_exact) pretool=($pretool_exact) posttool=($posttool_exact)"
        }
    } else {
        $failures = ($failures | append "hooks-generated-profile-owned")
        print $"clause hooks-generated-profile-owned: FAIL missing ($live_hooks)"
    }
    ...
    if ($live_hooks | path exists) {
        ...
        let hits = ($forbidden | where {|pattern| $raw | str contains $pattern })
        if ($hits | is-empty) {
            print "clause no-retired-or-inactive-hooks-paths: ok"
        } else {
            $failures = ($failures | append "no-retired-or-inactive-hooks-paths")
            print $"clause no-retired-or-inactive-hooks-paths: FAIL ($hits | str join ', ')"
        }
    }

    if ($failures | is-empty) {
        print $"ok codex provenance gate: ($live_config), ($live_rules), and ($live_hooks) are Yazelix-authored"
    } else {
