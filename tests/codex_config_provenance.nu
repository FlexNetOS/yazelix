...
def main [
    ...
    if ($profile_real | is-empty) or ($selector_real | is-empty) or ($selector_real != $profile_real) {
        ...
        if not ($codex_bin | str starts-with "/home/flexnetos/.nix-profile/") {
            print $"info: PATH selector is ($codex_bin), not the profile symlink"
        }
    }
    ...
    if ($live_rules | path exists) {
        ...
        if $marked and $headed {
            print "clause rules-generated-headed: ok"
        } else {
            $failures = ($failures | append "rules-generated-headed")
            print $"clause rules-generated-headed: FAIL generated-marker=($marked) heading=($headed)"
        }
    } else {
        $failures = ($failures | append "rules-generated-headed")
